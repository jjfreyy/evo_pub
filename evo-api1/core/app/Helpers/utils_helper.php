...
function generateRandomHex($len = 32) {
    return bin2hex(openssl_random_pseudo_bytes($len / 2));
}

function generateRandomToken($length = 8, $includePunctuation = false) {
  $digits = "0123456789";
  $digitsLength = strlen($digits) - 1;
  $chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  $charsLength = strlen($chars) - 1;
  $punctuations = "!@$%^&*()-=_+[]{};:,./?";
  $punctuationLength = strlen($punctuations) - 1;
  $token = "";
  for ($i = 0; $i < $length; $i++) {
    $rnd = rand(0, 1000);
    if ($includePunctuation) {
      if ($rnd < 300) {
        $token .= $digits[rand(0, $digitsLength)];
        continue;
      }

      if ($rnd >= 300 && $rnd <= 600) {
        $token .= $punctuations[rand(0, $punctuationLength)];
        continue;
      }
      ...
      if ($rnd < 400)
        $token .= $digits[rand(0, $digitsLength)];
      else $token .= $chars[rand(0, $charsLength)];
    }
  }
  return $token;
}
...
function toResponse($msg, $inputData, $errorData = [], $addOnInput = [], $excludeInput = []) {
    ...
    foreach ($inputData as $k => $v) {
        ...
        $feedbacks[$k] = [
            "val" => in_array($k, ["password", "confirmPassword", "pass", "confirmPass", "newPass"]) ? null : $v,
            "msg" => $errorData[$k] ?? null,
        ];
    }

    foreach ($addOnInput as $field) {
        if (isset($feedbacks[$field])) continue;

        $feedbacks[$field] = [
            "val" => null,
            "msg" => $errorData[$field] ?? null,
        ];
    }

    foreach ($excludeInput as $field) {
